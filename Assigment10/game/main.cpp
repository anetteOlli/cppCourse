...
public:
  ...
  void reset() {
    ...
    transform.setOrigin(btVector3(1.0, 0.1, 0.0));
    ground_sphere.body.setCenterOfMassTransform(transform);
    ground_sphere.body.setActivationState(0);

    transform.setOrigin(btVector3(0.9, 3.0, 0.0));
    falling_sphere.body.setCenterOfMassTransform(transform);
    falling_sphere.body.setActivationState(0);
    ...
    for (auto &cube : cubes) {
      cube.body.setActivationState(0);
    }
  }

  void drop_ball() {
      ground_sphere.body.activate();
      falling_sphere.body.activate();
      for (auto &cube : cubes) {
          cube.body.activate();
      }
  }

  void position_falling_sphere_horizontal(float horizontal_position){
      btTransform transform;
      horizontal = horizontal_position;
      transform.setOrigin(btVector3(horizontal, vertical, 0.0));
      falling_sphere.body.setCenterOfMassTransform(transform);
  }
  void position_falling_sphere_vertical(float vertical_position){
      btTransform transform;
  ...
  void start() {
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.0, 10.0)) {
        ...
        world.position_falling_sphere_horizontal(horizontal_position);
      }
      float vertical_position;
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &vertical_position, 0.0, 10.0)) {

        world.position_falling_sphere_vertical(vertical_position);
      }
      ImGui::End();
